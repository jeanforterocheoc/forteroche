...
class UserController extends SecureController
{
  ...
  */ 
  public function homepageUserAdmin()
  {
    ...
    $this->render('homepageUserAdmin', array('user' => $this->user, 'profil' => $nbProfil, 'chapter' => $nbChapter, 'comments' => $nbComments, 'report' => $nbReport));
  }

  /**
  * Edite la liste des profils utilisateurs enregistrés
  */
  public function usersProfilesList()
  {
    $userManager = new UserManager;
    $users = $userManager->listUsers();
    $this->render('usersProfilesList', array('usersProfilesList' => $users));
  } 
    ...
    if (isset($_POST['changeProfileUser'])) {
      if ($this->request->paramExist('changeUsername') && $this->request->paramExist('password') && $this->request->paramExist('passwordConfirm') && $this->request->paramExist('email')) {
        if ($_POST['password']  ==  $_POST['passwordConfirm']) {
          ...
          $this->redirection('user', 'usersProfilesList'); 
        } else {
          ...
          $messages = new Messages;
          $messages->setMsg('Veuillez remplir tous les champs !', 'error');
        } 
    }
  $this->render('changeprofilUser', array('oneUser' => $userProfil));
  }
    ...
    if (isset($_POST['login'])) {
      if ($this->request->paramExist('username') && $this->request->paramExist('password') && $this->request->paramExist('passwordConfirm') && $this->request->paramExist('email')) {
        if ($_POST['password'] == $_POST['passwordConfirm']) {
          ...
          $messages = new Messages;
          $messages->setMsg('Le profil a été créé !', 'success');
        } else {
            $messages = new Messages;
            $messages->setMsg('Les mots de passe ne correspondent pas !', 'error');
          } 
    ...
    if (isset($_POST['changeProfile'])) {
      if ($this->request->paramExist('username') && $this->request->paramExist('password') && $this->request->paramExist('passwordConfirm') && $this->request->paramExist('email')) {
        if ($_POST['password']  ==  $_POST['passwordConfirm']) {
          ...
          $userManager = new UserManager;
          $userManager->modifUser($id, $username, $password, $email);
        
          $messages = new Messages;
          $messages->setMsg('Les modifications ont été prises en compte!', 'success');
        } else {
            $messages = new Messages;
            $messages->setMsg('Les mots de passe ne correspondent pas!', 'error');
          }
